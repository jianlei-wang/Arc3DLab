import graphSourceDatas from './graph-datas.json';

export function loadZoomCharts() {
  // 创建许可证脚本
  ...
  graphDatas = graphSourceDatas;
  graphNodes = graphDatas.nodes;
  graphLinks = graphDatas.links;
}
...
export function query(str: string, clickNode?: any) {
  ...
  let _links = graphLinks.filter((ele) => nodeIds.includes(ele.from));
  ...
  for (let index = 0; index < maxLevel; index++) {
    ...
    const curLinks = graphLinks.filter((ele) => curNodeIds.includes(ele.from));
